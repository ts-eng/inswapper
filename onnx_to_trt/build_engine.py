...
class EngineBuilder:
    ...
    def set_mixed_precision(self):
        ...
        for i in range(self.network.num_layers):
            layer = self.network.get_layer(i)
            for j in range(65, 561, 45):
                if layer.name in [f'Mul_{j}', f'ReduceMean_{j + 1}', f'Add_{j + 3}', f'Sqrt_{j + 4}', f'Div_{j + 6}']:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="inswapper",
        choices=["inswapper"],
        help="Set the calibration image preprocessor to use, default: inswapper",
    )
